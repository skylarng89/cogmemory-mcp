...
/** Map a file extension to a display language name (no nested ternaries). */
...
export function registerCodeAnalysisTools(
    ...
    "get_code_snippet",
    {
      ...
      inputSchema: z.object({
        ...
        symbol_name: z
          ...
          .optional()
          .describe(
            "Symbol name to look up (exact match; first match if ambiguous)",
          ),
        file_path: z
          .string()
          .optional()
          .describe("Narrow search to this file path"),
        context_lines: z
          .number()
          .int()
          .min(0)
      ...
      async ({
        ...
        if (symbol_id !== undefined) {
          ...
          symbol = file_path
            ? (db
                .prepare(
                  `SELECT * FROM symbols WHERE symbol_name = ? AND file_path = ? AND ${projectPredicate()} LIMIT 1`,
                )
                .get(symbol_name, file_path, projectId) as
            ...
            content: [
              {
                ...
                text: JSON.stringify({
                  success: false,
                  message: `File not found on disk: ${absPath}`,
                }),
              },
            ],
          };
        ...
        const end = Math.min(
          lines.length,
          (symbol.end_line as number) + padding,
        );
  ...
  server.registerTool(
    "check_index_coverage",
    {
      ...
      inputSchema: z.object({
        root_dir: z
          .string()
          .optional()
          .describe("Root directory to check (default: workspace root)"),
        extensions: z
          .array(z.string())
          .optional()
          .describe(
    ...
    wrapHandler("check_index_coverage", async ({ root_dir, extensions }) => {
      ...
      const storedMap = new Map(
        storedIdx.map((f) => [f.file_path, f.mtime_ms]),
      );
      ...
      try {
        parseErrors = db
          .prepare(
            `SELECT file_path as path, error_message as error FROM index_errors WHERE ${projectPredicate()}`,
          )
          .all(projectId) as { path: string; error: string }[];
      } catch {
        /* table may not exist */
      }
      ...
      return jsonOk({
        ...
        coverage_pct:
          total > 0 ? Math.round((indexedCount / total) * 10000) / 100 : 100,
        by_language: byLang,
  ...
  server.registerTool(
    "find_dead_code",
    {
      ...
      inputSchema: z.object({
        entry_point_patterns: z
          ...
          .optional()
          .describe(
            "Symbol name patterns to exclude as entry points (default: main,index,cli,server,handler,setup,run)",
          ),
        exclude_tests: z
          .boolean()
          .optional()
          .describe(
            "Exclude test files *.test.* / *.spec.* / test_* (default: true)",
          ),
        exclude_exported: z
          .boolean()
          .optional()
          .describe(
            "Exclude exported symbols (default: true; requires migration 002)",
          ),
        file_pattern: z
          .string()
          .optional()
          .describe(
            "Only consider symbols whose file_path matches this substring",
          ),
        limit: z
          .number()
          .int()
    ...
    wrapHandler("find_dead_code", async (params) => {
      ...
      const entryConds = patterns
        .map(() => "s.symbol_name LIKE ?")
        .join(" OR ");
      ...
      const rows = db
        .prepare(sql)
        .all(projectId, ...entryParams, ...fileArgs, limit) as Record<
        string,
        unknown
      >[];
  ...
  server.registerTool(
    "query_graph",
    {
      ...
      inputSchema: z.object({
        start_symbol: z
          .string()
          .min(1)
          .describe("Starting symbol name or ID (string)"),
        edge_types: z
          .array(z.string())
          .optional()
          .describe("Edge types to traverse (default: all structural types)"),
        direction: z
        ...
        max_depth: z
          ...
          .max(20)
          .optional()
          .describe("Maximum depth (default: 5)"),
        limit: z
          .number()
          .int()
          .min(1)
          .max(1000)
          .optional()
          .describe("Maximum nodes returned (default: 100)"),
        filter: z
          .object({
            symbol_type: z
              .string()
              .optional()
              .describe("Only include symbols of this type"),
            file_pattern: z
              .string()
              .optional()
    ...
    wrapHandler("query_graph", async (params) => {
      ...
      const startId = Number.parseInt(params.start_symbol, 10);
      ...
      if (!Number.isNaN(startId)) {
        startSymbol = db
          ...
          .get(startId, projectId) as Record<string, unknown> | undefined;
      } else {
        startSymbol = db
          .prepare(
            `SELECT * FROM symbols WHERE symbol_name = ? AND ${projectPredicate()} ORDER BY updated_at DESC LIMIT 1`,
          )
          .get(params.start_symbol, projectId) as
          | Record<string, unknown>
          | undefined;
      }
      if (!startSymbol) {
          ...
          content: [
            {
              ...
              text: JSON.stringify({
                success: false,
                message: `Symbol not found: ${params.start_symbol}`,
              }),
            },
          ],
        };
      ...
      let nextIdExpr: string;
      if (direction === "inbound") {
        nextIdExpr = "e.from_symbol_id";
      } else if (direction === "outbound") {
        nextIdExpr = "e.to_symbol_id";
      } else {
        nextIdExpr =
          "CASE WHEN e.from_symbol_id = r.id THEN e.to_symbol_id ELSE e.from_symbol_id END";
      }
      ...
      const cte = `
        ...
        WHERE n.project_id = ? ${filterClauses}
        GROUP BY n.id
  ...
  server.registerTool(
    "analyze_impact",
    {
      ...
      inputSchema: z.object({
        changed_files: z
          .array(z.string())
          .optional()
          .describe("Override: explicit list of changed files (skip git diff)"),
        max_depth: z
          .number()
          .int()
          .min(1)
          .max(20)
          .optional()
          .describe("Max transitive depth (default: 5)"),
        edge_types: z
          .array(z.string())
          .optional()
          .describe("Edge types to follow (default: calls, imports)"),
        include_tests: z
          .boolean()
          .optional()
    ...
    wrapHandler("analyze_impact", async (params) => {
      ...
      const testFilter = includeTests
        ? ""
        : "AND file_path NOT LIKE '%.test.%' AND file_path NOT LIKE '%.spec.%'";
      const changedSymbolSql = `SELECT id, symbol_name, file_path, symbol_type FROM symbols WHERE file_path IN (${placeholders}) AND ${projectPredicate()} ${testFilter}`;
      const changedSymbols = db
        .prepare(changedSymbolSql)
        .all(...changedFiles, projectId) as {
        id: number;
        symbol_name: string;
        file_path: string;
        symbol_type: string;
      ...
      if (changedSymbols.length === 0) {
        return jsonOk({
          message:
            "Changed files found, but no indexed symbols match. Run index_codebase first.",
      ...
      const cte = `
        ...
        WHERE c.depth > 0 AND s.project_id = ? ${includeTests ? "" : "AND s.file_path NOT LIKE '%.test.%'"}
        GROUP BY s.id
      ...
      const impacted = db
        .prepare(cte)
        .all(...changedIds, ...types, maxDepth, projectId) as Record<
        string,
        unknown
      >[];
      ...
      return jsonOk({
        ...
        summary: {
          ...
          max_depth_reached: (impactWithPaths.at(-1)?.depth as number) ?? 0,
        },
      });
    }),
  );
  ...
  server.registerTool(
    "find_duplicates",
    {
      ...
      inputSchema: z.object({
        threshold: z
          ...
          .max(1)
          .optional()
          .describe("Jaccard similarity threshold (default: 0.7)"),
        min_tokens: z
          .number()
          .int()
          .min(1)
          .optional()
          .describe("Minimum tokens per symbol to compare (default: 10)"),
        recompute: z
          .boolean()
          .optional()
          .describe(
            "Recompute MinHash signatures from scratch (default: false — uses stored signatures)",
          ),
        file_pattern: z
          .string()
          .optional()
    ...
    wrapHandler("find_duplicates", async (params) => {
      ...
      const simEdges = db
        ...
        WHERE e.edge_type = ? AND ${projectPredicate("e")}
        ORDER BY e.created_at DESC
        LIMIT 500
      `,
        )
        .all(EDGE_TYPES.SIMILAR_TO, projectId) as Record<string, unknown>[];
      ...
      return jsonOk({
        duplicates: simEdges.map((e) => ({
          ...
          file_a: e.file_a,
          file_b: e.file_b,
          similarity: e.metadata
            ? JSON.parse(e.metadata as string).score
            : null,
          algorithm: e.metadata
            ? JSON.parse(e.metadata as string).algorithm
            : null,
          edge_id: e.id,
        })),
    ...
    "find_related",
    {
      ...
      inputSchema: z.object({
        symbol_name: z
          .string()
          .optional()
          .describe("Symbol name to find relations for"),
        symbol_id: z
          .number()
          .int()
          .optional()
          .describe("Symbol ID to find relations for"),
        threshold: z
          .number()
          .min(0)
          .max(1)
          .optional()
          .describe("Minimum score threshold (default: 0.3)"),
        limit: z
          .number()
          .int()
          .min(1)
    ...
    wrapHandler("find_related", async (params) => {
      ...
      if (params.symbol_id !== undefined) {
        target = db
          ...
          .get(params.symbol_id, projectId) as
          | Record<string, unknown>
          | undefined;
        ...
        target = db
          ...
          .get(params.symbol_name, projectId) as
          | Record<string, unknown>
          | undefined;
      }
      if (!target) {
      ...
      const callerSql = `
        ...
        LIMIT 200
      `;
      const sharedCallers = db.prepare(callerSql).all(targetId, targetId) as {
        shared_id: number;
      }[];
      ...
      const importSql = `
        ...
        LIMIT 200
      `;
      const sharedImports = db.prepare(importSql).all(targetId, targetId) as {
        shared_id: number;
      }[];
      ...
      if (target.file_path) {
        const sameFile = db
          .prepare(
            "SELECT id FROM symbols WHERE file_path = ? AND id != ? AND symbol_type != 'file' LIMIT 50",
          )
          .all(target.file_path, targetId) as { id: number }[];
      ...
      `;
      const existingSimilar = db
        .prepare(similarSql)
        .all(targetId, EDGE_TYPES.SIMILAR_TO, targetId, targetId) as {
        related_id: number;
      }[];
      ...
      const insertEdge = hasMetadata
        ? db.prepare(
            `INSERT OR IGNORE INTO edges (project_id, from_symbol_id, to_symbol_id, edge_type, metadata) VALUES (?, ?, ?, ?, ?)`,
          )
        : db.prepare(
            `INSERT OR IGNORE INTO edges (project_id, from_symbol_id, to_symbol_id, edge_type) VALUES (?, ?, ?, ?)`,
          );
      ...
      db.transaction(() => {
        for (const [symId, { score, reasons }] of candidates) {
          ...
          const sym = db
            .prepare(
              `SELECT id, symbol_name, file_path, symbol_type FROM symbols WHERE id = ? AND ${projectPredicate()}`,
            )
            .get(symId, projectId) as Record<string, unknown> | undefined;
          if (sym) {
            related.push({
              symbol_id: sym.id,
  ...
  server.registerTool(
    "semantic_code_search",
    {
      ...
      inputSchema: z.object({
        ...
        threshold: z
          ...
          .min(0)
          .optional()
          .describe("Minimum score threshold (default: 0.0)"),
        file_pattern: z
          .string()
          .optional()
          .describe("Only search symbols whose file_path matches"),
        symbol_type: z
          .string()
          .optional()
    ...
    wrapHandler("semantic_code_search", async (params) => {
      ...
      if (!hasTokens) {
        return jsonOk({
          message:
            "symbol_tokens table not available. Run index_codebase first to populate the TF-IDF index.",
          results: [],
      ...
      if (queryTokens.length === 0) {
        return jsonOk({
          ...
          total_matches: 0,
          message:
            "Query contained no meaningful tokens after stopword removal",
        });
      }
      ...
      if (totalSymbols === 0) {
        return jsonOk({
          ...
          backend: "tfidf",
          total_matches: 0,
          message: "No tokens indexed. Run index_codebase first.",
        });
      }
      ...
      // Compute TF-IDF scores per symbol manually (token-specific IDF)
      const tokenRows = db
        .prepare(
          `
      ...
      const rankedCandidates = [...scores.entries()]
        .filter(([, score]) =>
          maxScore > 0 ? score / maxScore >= threshold : false,
        )
        .map(([id, score]) => ({
          id,
      ...
      // Fetch symbol details and snippets (scoped to the active project)
      ...
      for (const candidate of rankedCandidates) {
        const sym = db
          ...
          .get(candidate.id, projectId) as Record<string, unknown> | undefined;
        if (!sym) continue;
        ...
        try {
          ...
          if (existsSync(absPath)) {
            ...
            const endLine = Math.min(
              (sym.end_line as number) ?? startLine,
              lines.length,
            );
            snippet = lines
              .slice(startLine - 1, Math.min(endLine, startLine + 10))
              .join("\n")
              .slice(0, 500);
          }
        } catch {
      ...
      for (let j = i + 1; j < ids.length; j++) {
        ...
        if (sim >= threshold) {
          ...
          if (hasMetadata) {
            ...
            insertEdge.run(projectId, ids[i], ids[j], EDGE_TYPES.SIMILAR_TO);
          }
          count++;
        }
      }
