...
export const GenerateCodemapSchema = z.object({
  entry_symbol: z.string().describe("Starting symbol name for the codemap"),
  max_hops: z
    .number()
    .int()
    .min(1)
...
export const AnnotateSymbolSchema = z
  .object({
    ...
    annotation: z.string().describe("Narrative text to attach"),
  })
  .refine(
  ...
  server.registerTool(
    ...
    async ({ symbol_id, trace_id, annotation }) => {
      ...
      const stmt = db.prepare(`
        ...
        VALUES (?, ?, ?)
      `);
      const result = stmt.run(symbol_id ?? null, trace_id ?? null, annotation);
